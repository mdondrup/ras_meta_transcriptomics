...
# Debug helper: writes to stderr() so messages appear in the Snakemake
# console/log instead of being captured by knitr into the HTML document.
dbg <- function(...) cat(..., "\n", file = stderr())
```


```{r}
# Check if we are running in Snakemake or not, and if not, load the saved snakemake object to get the parameters and input paths.
if (!exists('snakemake'))
...
dbg("saved snakemake object to de_analysis.restart.rda")
...
dbg("Dimensions of raw.mat:", dim(raw.mat))
dbg("Summary of raw.mat:", summary(raw.mat))
zero.pct <- 100*sum(raw.mat == 0)/prod(dim(raw.mat))

dbg("% zero unfiltered:", zero.pct)
zero.pct
...
dbg("Dimensions of filtered raw.mat:", dim(raw.mat))
zero.pct <- 100*sum(raw.mat == 0)/prod(dim(raw.mat))
dbg("% zero in filtered raw counts:", zero.pct)
zero.pct
# fix colnames
...
```{r}
dbg("Importing annotation from", gff.path)
...
```{r}
dbg("Reading sample metadata from", meta.data.path)
...
dbg("Column names of col.data:", names(col.data))
...
dbg("Running DESeq2 with", nthreads, "threads")
...
dbg("Finished DESeq2; result names:", resultsNames(dds)) # lists the coefficients
